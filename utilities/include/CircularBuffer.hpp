...
template<typename T>
void initialize_buffer(std::size_t size, std::vector<T>& buffer, std::size_t& mask){
...
template<typename T, typename BufferT>
T& CircularBuffer<T, BufferT>::front() 
{
    return (*this)[0];
} 
...
template<typename T, typename BufferT>
T& CircularBuffer<T, BufferT>::back() 
{
    return (*this)[size()-1];
} 


template<typename T, typename BufferT = std::vector<T>, typename IndexType = float>
...
public:
    ...
    T const& operator[](IndexType index) const{
        return linear_interpolation(static_cast<base_type const&>(*this),index); 
    }
};
