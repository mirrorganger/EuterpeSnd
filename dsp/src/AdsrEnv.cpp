...
void AdsrEnv::configure(const AdsrEnv::Parameters &param) {
  ...
  _releaseSection.overShoot =
      utilities::fromDecibelsToGain(DECAY_RELEASE_OVERSHOOT_DB);
  _decaySection.overShoot =
      utilities::fromDecibelsToGain(DECAY_RELEASE_OVERSHOOT_DB);
  updateRates();
}
...
void AdsrEnv::setAttackTime(
    std::chrono::duration<double, std::milli> attackTime) {
  _parameters.attackTime = attackTime;
  updateRates();
}

void AdsrEnv::setReleaseTime(
    std::chrono::duration<double, std::milli> releaseTime) {
  _parameters.releaseTime = releaseTime;
  updateRates();
}

void AdsrEnv::setDecayTime(
    std::chrono::duration<double, std::milli> decayTime) {
...
void AdsrEnv::release() { _stm.transition(NoteOffEvt{}); }

void AdsrEnv::reset() { _envelopeValue = MINIMUM_LEVEL; }
...
void AdsrEnv::process(utilities::AudioBuffer<float> &buffer) {
  auto multVisitor = Overloaded{
      [&](Idle) {
        utilities::AudioBufferTools::clearRawBuffer(
            buffer.getWritePointer(),
            buffer.getNumberOfChannels() * buffer.getFramesPerBuffer());
      },
      [&](Sustain) {
        utilities::AudioBufferTools::multiplyRawBuffer(
            buffer.getWritePointer(),
            buffer.getNumberOfChannels() * buffer.getFramesPerBuffer(),
            _envelopeValue);
      },
      [&](auto otherState) {
        ...
        for (int sample_i = 0; sample_i < buffer.getFramesPerBuffer();
             ++sample_i) {
...
void AdsrEnv::updateRates() {
  _attackSection.advanceMultiplier =
      getRateMult(_attackSection.overShoot, 1.0 + _attackSection.overShoot,
                  _parameters.attackTime);
  _attackSection.baseValue = (1.0 + _attackSection.overShoot) *
                             (1.0 - _attackSection.advanceMultiplier);
  _decaySection.advanceMultiplier =
      getRateMult(_decaySection.overShoot, 1.0 + _decaySection.overShoot,
                  _parameters.decayTime);
  _decaySection.baseValue =
      (_parameters.sustainLevel - _decaySection.overShoot) *
      (1.0 - _decaySection.advanceMultiplier);
  _releaseSection.advanceMultiplier =
      getRateMult(_releaseSection.overShoot, 1.0 + _releaseSection.overShoot,
                  _parameters.releaseTime);
  _releaseSection.baseValue =
