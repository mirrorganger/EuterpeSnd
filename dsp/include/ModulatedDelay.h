...
public:
  ModulatedDelay(float sampleRate_hz, float lfoFreq_hz, float delay_ms,
                 ...
                 Lfo::WaveFormT type = Lfo::WaveFormT::TRIANGLE)
      : _lfo(sampleRate_hz, lfoFreq_hz, Lfo::WaveFormT::TRIANGLE),
        _sampleRate_hz(sampleRate_hz), _baseDelay_ms(delay_ms),
        _lfoDepth(lfoDepth), _feedback(feedback) {
  ...
  void process(DataType *const *buffer, size_t numberOfChannels,
               size_t numberOfSamples) {
    auto out = DataType{};
    _lfo.getBlock(_modulationValues.data(), _modulationValues.size());
    std::ranges::for_each(_modulationValues, [&](float &val) {
      auto modulation =
          std::max(1.0F, (_maxModulation_ms * val * _lfoDepth + _baseDelay_ms));
    ...
    for (size_t channel_i = 0; channel_i < _delays.size(); ++channel_i) {
      for (size_t sample_i = 0; sample_i < numberOfSamples; ++sample_i) {
        ...
        buffer[channel_i][sample_i] =
            _dryWetMix * in + (1.0F - _dryWetMix) * dn;
      }
    }
  }

  void setDelay(float delay_ms) { _baseDelay_ms = delay_ms; }

  void setFeedback(float feedback) { _feedback = feedback; }
  void setDryWetMix(float dryWet) { _dryWetMix = dryWet; }

  void setLfoDepth(float lfoDepth) { _lfoDepth = lfoDepth; }

  void setLfoFreq(float lfoFreq) { _lfo.update(_sampleRate_hz, lfoFreq); }
